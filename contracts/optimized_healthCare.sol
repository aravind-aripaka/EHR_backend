...
contract optimized_healthCare {
  ...
  mapping (address => Unique) patient_Unique_info;
  mapping (address => Unique) doctor_Unique_info;


  struct Unique{
    address id;
    uint64 Unique_number;
    string name;
    string DOB;
    uint24 pincode;
  }
  ...
  //setting the owner
  constructor() public {
    owner = 0xf6e1aDDd978fb74acC645eA8BCB49DC468f1834c;
  }
  ...
  function signupPatient(string memory _name, string memory _contact, string memory _gender) public {
    ...
    Unique memory a = patient_Unique_info[msg.sender];
    //Check if the patient already exists by address
    require(!(p.id > address(0x0)));
    //Add patient to blockchain
    require(a.Unique_number > 0);

    patients[msg.sender] = patient({name:_name, DOB: a.DOB, id:msg.sender, Unique_number: a.Unique_number, gender:_gender, contact_info:_contact, files:new bytes32[](0), doctor_list:new address[](0)});
  ...
  function signupDoctor(address _id, string memory _name, string memory _contact, string memory _specialization) public {
      ...
      Unique memory a = doctor_Unique_info[_id];
      ...
      doctors[_id] = doctor({name:_name, id:_id, contact:_contact, Unique_number: a.Unique_number, specialization: _specialization, patient_list:new address[](0)});
  ...
  function grantAccessToDoctor(address doctor_id) public checkPatient(msg.sender) checkDoctor(doctor_id) {
      ...
      );// this means doctor already been access
  ...
  function checkDoctorInfo(uint64 _Unique_number) public view returns(bool, address) {
    
    doctor memory d = doctors[msg.sender];
    Unique memory a = doctor_Unique_info[msg.sender];
    if (a.Unique_number == _Unique_number){
      return (true, d.id);
    }
  ...
  function checkPatientInfo(uint64 _Unique_number) public view returns(bool, address) {
    patient memory p = patients[msg.sender];
    Unique memory a = patient_Unique_info[msg.sender];
    if (a.Unique_number == _Unique_number){
      return (true,p.id);
  ...
  function addDoctorUniqueInfo(address _doc, string memory _name, string memory _DOB, uint24 _pincode, uint64 _Unique_number) public {
    Unique memory a = doctor_Unique_info[_doc];
    require(a.Unique_number == 0);
    doctor_Unique_info[_doc] = Unique({id:_doc, pincode: _pincode, name: _name, DOB: _DOB, Unique_number: _Unique_number});
  }

  function addPatientUniqueInfo(address _pat, string memory _name, string memory _DOB, uint24 _pincode, uint64 _Unique_number) public {
    Unique memory a = patient_Unique_info[_pat];
    require(a.Unique_number == 0);
    patient_Unique_info[_pat] = Unique({id:_pat, pincode: _pincode, name: _name, DOB: _DOB, Unique_number: _Unique_number});
  }
